...
static func move_with_decceleration(
	...
	var new_velocity = Vector2(real_velocity.x + d_velocity.x, real_velocity.y + d_velocity.y).limit_length(max(horizontal_movement_store.max_velocity, real_velocity.length()))
