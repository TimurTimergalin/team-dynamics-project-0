...
## Применяется замедление: при отсутствии ввода скорость уменьшается, при изменении скорости в противоположном направлении
...
static func move_with_decceleration(
    ...
    var target_velocity = (
        horizontal_movement_store.direction * horizontal_movement_store.max_velocity
    )
    ...
    var derivative = (
        real_velocity.x * d_velocity_direction.x + real_velocity.y * d_velocity_direction.y
    )
    ...
    var new_velocity = (
        Vector2(real_velocity.x + d_velocity.x, real_velocity.y + d_velocity.y)
        . limit_length(max(horizontal_movement_store.max_velocity, real_velocity.length()))
    )
...
## Расчитывает изменение скорости тела в зависимости от напрвления, заданного управляющей машиной
## Замедление не применяется: при отсутствии ввода горизонтальная скорость не меняется, ускорение одинаковое во всех направлениях
...
static func handle_slided(
    ...
    var rotation := 0.0 if old_velocity.is_zero_approx() else old_velocity.angle_to(new_velocity)  # Тождественный поворот
    var scale := (
        1.0 if old_velocity.is_zero_approx() else new_velocity.length() / old_velocity.length()
    )
